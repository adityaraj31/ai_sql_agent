...
def analyze_data_for_chart(question: str, df: pd.DataFrame) -> Optional[Dict[str, Any]]:
    ...
    if df.empty or (
        len(df) < 2 and len(df.select_dtypes(include=["number"]).columns) < 2
    ):
        return None
    ...
    try:
        response = chain.invoke(
            {"question": question, "columns": columns, "data_sample": data_sample}
        )
...
def render_chart(df: pd.DataFrame, config: dict):
    ...
    if x == "__columns__" and y == "__values__" and len(df) == 1:
        ...
        df_long = pd.DataFrame(
            {"Metric": metric_cols, "Value": [df.iloc[0][col] for col in metric_cols]}
        )
        x, y = "Metric", "Value"
        df = df_long
    ...
    try:
        if chart_type == "bar":
            ...
            fig = px.line(
                df, x=x, y=y, title=title, markers=True, template="plotly_dark"
            )
        elif chart_type == "pie":
